...
const Query = {
  ...
  grade: async (parent, { studentID, subject }) => {
    let grade = await GradeModel.find({
      $and: [{ studentID: studentID }, { subject: subject }],
    });
    return grade;
  },
  file: async (parent) => {
  ...
  quiz: async (parent, { name, courseID, studentID }) => {
    let box = await ChatBoxModel.findOne({
      name,
      courseID,
      attendants: { $in: [studentID] },
    });
    let quiz = await QuizModel.findOne({
      chatbox: box._id,
    });
    console.log(box);
    // if(!box) box = await new QuizModel({chatbox: {name, courseID}})
    return quiz;
  },
};
